...
agdaMode = paragraph
  ...
  , en "A REPL interacts with Agda CLI interface, simulating Ltac in Coq"

  , itemize
    [ cn "\\item 通过 tokio 和 serde 实现 IO，并在 Agda 编译器中实现 json 接口。"
    , en "\\item Implements IO via tokio and serde along with the json interface in Agda compiler."

    , cn "\\item 支持在 REPL 中进行绝大多数原本在 Emacs mode 里实现的操作，例如 case split、显示表达式与洞的类型等。"
    , en "\\item Supports most actions available in the Emacs mode, like case-split, show expression and goal type, etc."
    ]
  ]
